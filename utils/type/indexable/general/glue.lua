...
function glue(base, addition, opts)
  ...
  elseif isEmptyTable(addition) then
    ...
    if isListOrEmptyTable(addition) then
      if isListOrEmptyTable(base) then -- glue(list, list2)
      ...
      if isListOrEmptyTable(base) then -- glue(list, assocarr) if the base is a list, and we're gluing an assoc arr, we're just gonna treat it as a value to append
        base = append(base, addition, opts)
        ...
        for k, v in prs(addition) do
          if type(v) == "table" and not isListOrEmptyTable(v) then -- we could recurse
            if 
              type(base[k]) ~= "table" 
              or isListOrEmptyTable(base[k])
              or not opts.recurse 
              or (
