...
function defaultOpts(opts, retdefault)
  ...
  if type(opts.args) == "string" then
    opts.args = transf.string.char_array(opts.args)
  end
  if type(opts.ret) == "string" then
    if opts.ret == "boolean" then
      -- no-op
    else
      opts.ret = transf.string.char_array(opts.ret)
    end
  end
...
function getIterator(opts)
  return
    function(tbl) 
      return get.indexable.key_value_stateless_iter(
        tbl or opts.limit or {}, opts.start, opts.stop, opts.last and -1 or 1) 
      end
end
...
function addToRes(itemres,res,opts,k,v)
  ...
  if opts.flatten and type(newval) == "table" then
    for resk, resv in transf.table.stateless_key_value_iter(newval) do
