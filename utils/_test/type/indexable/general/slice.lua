...
local result9 = slice(test_str, {start = {_r = "^h", _invert = true}, stop = 5})
assertMessage(result9, "ello")

-- Test 10: Slice using a conditionSpec on a table
local result10 = slice(test_tbl, {start = {_exactly = "a", _invert = true}, stop = 3})
...
assertMessage(result18, "h_lo_")

-- Test 19: Slice using fill on a table
local result19 = slice(test_tbl, {start = 1, stop = 5, step = 2, fill = "x"})
assertMessage(result19, {"a", "x", "c", "x", "e"})

-- Test 20: Slice using last_start and last_stop on a string
local result20 = slice(test_str, {start = {_r = "l", last_start = true}, stop = {_r = "o", last_stop = true}})
...
local result22 = slice(test_str, {start = 1, stop = 5, step = 2, fill = "_", sliced_indicator = "..."})
assertMessage(result22, "h_lo_...")
