...
local diff = tonumber(run("datediff @" .. nextRun1 .. " now -f '%S'"))
assertMessage(diff < 300, true)
local mins = tonumber(run("date -d @" .. nextRun1 .. " +'%M'"))
...
local diff = tonumber(run("datediff @" .. nextRun2 .. " now -f '%H'"))
assertMessage(diff < 24, true)
local hours = tonumber(run("date -d @" .. nextRun2 .. " +'%H'"))
...
local diff = tonumber(run("datediff @" .. nextRun3 .. " now -f '%d'"))
assertMessage(diff < 7, true)
local day = tonumber(run("date -d @" .. nextRun3 .. " +'%w'"))
assertMessage(day, 1)