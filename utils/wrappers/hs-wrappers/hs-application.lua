...
function getMenuItemList(application_name)
  ...
  local filtered = listFilter(flattened, function (v) return v.AXTitle ~= "" end)
  local filtered_with_ref =mergeAssocArrWithAllListElements(filtered, { application = application })
  local mapped_to_items = mapValueNewValue(filtered_with_ref, function (v) return CreateTable(v) end)
