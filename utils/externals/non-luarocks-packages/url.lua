...
function M:parseSucceeded()
  return self.host ~= nil -- technically, URLs without a host are valid, and the scheme is the only necessary and distinct part of an URL, but for our purposes, we need a host
end

function parseGuessScheme(url)
  local try_parsed = M.parse(url)
  if try_parsed:parseSucceeded() then
    return try_parsed
  else
    local try_parsed_2 = M.parse("https://" .. url)
    if try_parsed_2:parseSucceeded() then
      return try_parsed_2
    else
      return nil
    end
